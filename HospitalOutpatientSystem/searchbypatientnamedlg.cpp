//***************************************************
//文件名 ：      searchbypatientnamedlg.cpp
//版本 ：                   5.0
...
#include "ui_searchbypatientnamedlg.h"

SearchByPatientNameDlg::SearchByPatientNameDlg(QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::SearchByPatientNameDlg)
{
    ui->setupUi(this);
    ...
    QFile FindOutpatientFile(strFilePath);    //创建QFile对象FindOutpatientFile，并将strFilePath作为构造函数的参数传入,初始化文件路径
    FindOutpatientFile.resize(0);           //清空文件中所有的信息
}
...
**********************************************************/
void SearchByPatientNameDlg::SetRowData(QVector<CHospitalOutpatient> Outpatients,int index)
{
...
SearchByPatientNameDlg::~SearchByPatientNameDlg()
{
    delete ui;
}
